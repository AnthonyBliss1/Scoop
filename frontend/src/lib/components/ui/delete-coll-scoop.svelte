<script lang="ts">
  ...
  let { showDelete = $bindable<boolean>(), mode = $bindable<Item>() } = $props<{
    showDelete: boolean;
    mode: Item;
  }>();
  ...
  async function deleteScoop() {
    const target = app.currentScoop;
    const coll = app.currentCollection;

    // this shouldnt fire, just in case
    if (target.name === "temp" || coll.name === "temp") {
      toast.warning("Cannot delete 'temp' collection");
      return;
    }
    ...
    try {
      const ok = await ScoopService.DeleteScoop(coll, target);

      if (ok) {
        deleting = false;
        showDelete = false;

        // instead of app.reset i need to remove the scoop from the collection in the app data
        app.currentCollection.scoops = coll.scoops.filter((scoop) => scoop.name !== target.name);
        app.allScoops = app.currentCollection.scoops;
        app.currentScoop = app.currentCollection.scoops[0];

        toast.success(`Deleted '${target.name}' Scoop`);
      }
    } catch (error) {
      console.error(error);
      deleting = false;
...
{#if mode === "Collection"}
  ...
  <!-->Deleting Current Scoop<-->
{:else if mode === "Scoop"}
  <div class="border-border bg-background flex flex-col gap-5 rounded-sm border p-5">
    <p class="flex items-center justify-center">
      Are you sure you want to delete '{app.currentScoop.name}'?
    </p>
    ...
    <div class="flex w-full flex-row items-center justify-center gap-10">
      ...
      disabled:cursor-not-allowed disabled:opacity-50 disabled:hover:bg-transparent disabled:hover:text-red-500"
        disabled={deleting}
        onclick={deleteScoop}>Delete</button
      >
